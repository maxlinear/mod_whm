...
swl_rc_ne whm_mxl_createMLVap(T_AccessPoint* pAPlink1, T_AccessPoint* pAPlink2) {
     ...
     /* set mlo_enable flag for both the VAPs*/
     wld_ap_hostapd_setParamValue(pAPlink1, "mlo_enable", "1", "Setting mlo_enable flag for link1");
     wld_ap_hostapd_setParamValue(pAPlink2, "mlo_enable", "1", "Setting mlo_enable flag for link2");
     swl_rc_ne ret = whm_mxl_toggleHapd(pRadLink1);
     if (ret)
         return ret;
     return whm_mxl_toggleHapd(pRadLink2);
}
...
void whm_mxl_destroyMLVap(T_AccessPoint* pAP) {
    ...
    /* Get Radio of Each link */
    T_Radio* pRad   = pAP->pRadio;
    T_Radio* sibRad = sibAP->pRadio;
    ...
    swl_typeCharPtr_commitObjectParam(link2MloObj, "ApMldMac", "");
		/* To Deinitialize MLO in hostapd ctrl_iface disable and enable should be called */
    SAH_TRACEZ_INFO(ME, "MLO: Disabling MLO Associated Radios to Deinit MLO in Hostapd");
    wld_rad_hostapd_disable(pRad);
    wld_rad_hostapd_disable(sibRad);
    wld_ap_hostapd_setParamValue(pAP, "mlo_enable", "0", "Disabling mlo_enable flag for link1");
    wld_ap_hostapd_setParamValue(sibAP, "mlo_enable", "0", "Disabling mlo_enable flag for link2");
